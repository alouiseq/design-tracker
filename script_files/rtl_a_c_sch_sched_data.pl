#!/router/bin/perl
## This script extracts RTL Stage A-C completion dates for SCH Blocks, converts the standard date format into # of days, 
...
my @blk_buffer = `ls DEQ*blk.txt PAR*blk.txt SIF*blk.txt SOR*blk.txt SRT*blk.txt RCS*blk.txt RCM*blk.txt REM*blk.txt`;
...
my $txtfile = "rtl_a_c_sch_sched_data.txt";
