...
# retrieve the FP and new DFT_RTL_Status_date from the argument
# form received must be FP/mod/DFT_RTL_Status_date
# e.g. XYX/mod/NA 
$_ = $ARGV[0];
s/\/.*//;
$fp = $_;
...
$_ = $ARGV[0];
s/^.*mod\///;
$new_DFT_RTL_Status_date = $_;
#printf ("New DFT_RTL_Status_date is %s\n",$new_DFT_RTL_Status_date);
...
# while searching for the dft_rtl_status_date variable
# while changing the dft_rtl_status_date to the new value
# append each line to $output
foreach $line (@lines) {
  # look for line containing dft_rtl_status variable
  $vari = $line;
  $vari =~ s/ = .*$//;
  if ($vari eq "dft_rtl_status_date\n"){
     $output .= 'dft_rtl_status_date = "'; 
     $output .= $new_DFT_RTL_Status_date; 
     $output .= '"'; 
     $output .= "\n"; 
  }else{
     $output .= $line;
