...
# retrieve the BLK, new RTL_STAGE_completion_date, and RTL Stage from the argument
# form received must be BLK/mod/date/stg/stage
# e.g. XYX/mod/date/stg/stage
$_ = $ARGV[0];
s/\/.*//;
$blk = $_;
#printf ("BLK is %s\n",$blk);
...
s/.*mod\///;
s/\/+stg.*//;
$new_RTL_STAGE_completion_date = $_;
#printf ("New RTL_STAGE date is %s\n",$new_RTL_STAGE_completion_date);
$_ = $ARGV[0];
s/.*stg\///;
$stage = $_;
#printf ("Stage is %s\n",$stage);
...
foreach $line (@lines) {
  ...
  $vari = $line;
  $vari =~ s/ = .*$//;
  if ($vari eq "rtl_stage".$stage."_status_completion_date\n"){
     $output .= 'rtl_stage'.$stage.'_status_completion_date = "'; 
     $output .= $new_RTL_STAGE_completion_date; 
     $output .= '"'; 
     $output .= "\n"; 
  }else{
     $output .= $line;
