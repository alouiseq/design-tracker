...
# retrieve the BLK and new RTL_Review_Status from the argument
# form received must be BLK/mod/RTL_Review_Status
# e.g. XYX/mod/NOTUP
$_ = $ARGV[0];
s/\/.*//;
$blk = $_;
...
$_ = $ARGV[0];
s/^.*mod\///;
$new_RTL_Review_Status = $_;
#printf ("New RTL_Review_Status is %s\n",$new_RTL_Review_Status);
...
# while searching for the rtl_design_review_checklist_status variable
# while changing the rtl_design_review_checklist_status to the new rtl_design_review_checklist_status
# append each line to $output
foreach $line (@lines) {
  # look for line containing rtl_design_review_checklist_status variable
  $vari = $line;
  $vari =~ s/ = .*$//;
  if ($vari eq "rtl_design_review_checklist_status\n"){
     $output .= 'rtl_design_review_checklist_status = "'; 
     $output .= $new_RTL_Review_Status; 
     $output .= '"'; 
     $output .= "\n"; 
  }else{
     $output .= $line;
